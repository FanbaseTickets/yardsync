    ...
    if (count > 0) {
      if (seatItem) {
        ...
        try {
          ...
          // Race: a concurrent accept created the seat item first (Stripe rejects
          // a duplicate price on one sub). Re-fetch + reconcile the quantity to the
          // true active count so we never under-bill.
          const sub2  = await stripe.subscriptions.retrieve(subId)
