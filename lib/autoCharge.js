 ...
 *   { ok:false, code }  — 'no_amount' | 'no_card' | 'no_connect' | 'no_platform_card' | 'error'
 */
export async function chargeClientForVisit({ contractor, gardenerUid, client, clientId, visit, baseUrl }) {
  ...
  // Free-access activation invariant (mirrors the invoice + quote-deposit paths):
  // a 'free_until_paid' contractor with no PLATFORM card on file must NOT collect
  // money off-session — their first paid charge triggers $39/mo activation, which
  // silently fails with no card, leaving the client charged but the sub unbilled.
  // Retryable (pre-PI): once they add a card, the next run charges cleanly.
  if (contractor?.subscriptionStatus === 'free_until_paid' && contractor?.pmOnFile !== true) {
    return { ok: false, code: 'no_platform_card' }
  }
