...
// ── Crew Tier: membership reads (client SDK; owner-scoped by the rules) ──────
export async function getTeamMemberships(ownerUid) {
  // Everyone in MY business (pending invites + active + removed).
  const q = query(collection(db, 'memberships'), where('businessUid', '==', ownerUid))
  const snap = await getDocs(q)
  return snap.docs.map(d => ({ id: d.id, ...d.data() }))
}
export async function getMyCrews(memberUid) {
  // Businesses I'm an active member (Worker) of.
  const q = query(collection(db, 'memberships'), where('memberUid', '==', memberUid), where('status', '==', 'active'))
  const snap = await getDocs(q)
  return snap.docs.map(d => ({ id: d.id, ...d.data() }))
}
// Schedules assigned to me in a crew I work in (owner-scoped: gardenerUid ==
// the crew owner). Union of the single-assign field (assignedTo == me) and the
// multi-assign array (assignedTeam array-contains me) — both provably owner-safe
// by their filters, so neither query trips the Worker read rule. Deduped by id.
export async function getWorkerSchedules(crewOwnerUid, workerUid, monthStart, monthEnd) {
  const base = collection(db, 'schedules')
  const q1 = query(base, where('gardenerUid', '==', crewOwnerUid), where('assignedTo', '==', workerUid))
  const q2 = query(base, where('gardenerUid', '==', crewOwnerUid), where('assignedTeam', 'array-contains', workerUid))
  const [s1, s2] = await Promise.all([getDocs(q1), getDocs(q2)])
  const byId = {}
  for (const d of [...s1.docs, ...s2.docs]) byId[d.id] = { id: d.id, ...d.data() }
  return Object.values(byId).filter(s => s.serviceDate >= monthStart && s.serviceDate <= monthEnd)
}
...
export async function getClientInvoices(gardenerUid, clientId) {
  // Filter by gardenerUid too so the query is owner-scoped — required once the
  // Firestore rules restrict invoices to the owner (same 2-equality pattern the
  // schedule queries use). Callers pass their own uid.
  const q = query(collection(db, 'invoices'), where('gardenerUid', '==', gardenerUid), where('clientId', '==', clientId))
  const snap = await getDocs(q)
  return snap.docs.map(d => ({ id: d.id, ...d.data() }))
}
...
export async function getMostRecentSchedule(gardenerUid, clientId) {
  // Check scheduled jobs first, then completed — so materials are found either way
  const q = query(
    collection(db, 'schedules'),
    where('gardenerUid', '==', gardenerUid),
    where('clientId', '==', clientId),
  ...
  // Prefer scheduled jobs, fall back to completed, sorted by most recent date
  const withMaterials = docs.filter(d => d.materials?.length > 0)
  if (withMaterials.length > 0) {
    return withMaterials.sort((a, b) => b.serviceDate.localeCompare(a.serviceDate))[0]
  }
...
export async function saveICalEvent(scheduleId, eventData) {
  await setDoc(doc(db, 'icalEvents', scheduleId), {
    ...eventData,
