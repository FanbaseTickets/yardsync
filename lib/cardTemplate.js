...
export async function composeSocial(canvas, { width, height, data, qrUrl, lang }) {
  ...
  // ── Avatar (logo preferred, else headshot, else initials) ──
  const avatar = data.logoImg || data.headshotImg
  if (avatar) {
    drawCircleImage(ctx, avatar, cx, y + avatarR, avatarR)
  } else {
    ctx.fillStyle = accent
    ctx.beginPath(); ctx.arc(cx, y + avatarR, avatarR, 0, Math.PI * 2); ctx.fill()
    ctx.fillStyle = '#ffffff'
    ctx.font = `700 ${Math.round(avatarR * 0.9)}px ${FONT}`
    ctx.fillText(getInitials(data.businessName), cx, y + avatarR + avatarR * 0.32)
  }
  y += avatarH
