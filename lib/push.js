...
export async function sendPush(gardenerUid, { title, body, url, tag, actions, navigateUrl, scheduleId }) {
  try {
    ...
    // Forward optional fields the service worker uses: `tag` (dedupe/replace),
    // `actions` (lock-screen buttons), and per-action data (`navigateUrl` for the
    // maps deep-link, `scheduleId` for mark-complete).
    const payload = JSON.stringify({ title, body, url, tag, actions, navigateUrl, scheduleId })
    const dead = []
