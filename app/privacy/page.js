      ...
      <section className="space-y-6 text-gray-700 text-sm leading-relaxed">
        ...
        <div>
          ...
          <ul className="list-disc pl-5 mt-2 space-y-1">
            ...
            {/* PENDING LEGAL REVIEW (added 2026-07-05): property type + precise geolocation (Geoapify) + crew/worker account data. */}
            <li>{es ? 'Tipo de propiedad de un cliente o prospecto (residencial, comercial, HOA u otro), cuando se proporciona' : 'The property type of a client or prospect (residential, commercial, HOA, or other), when provided'}</li>
            <li>{es ? 'Datos de ubicación. Al escribir una dirección en un formulario de captación, el texto de la dirección se envía a nuestro proveedor de mapas (Geoapify) para sugerir coincidencias. Si usted toca "Usar mi ubicación actual", su navegador solicita su permiso y, si lo concede, la ubicación precisa de su dispositivo se envía a Geoapify para convertirla en una dirección. Esta función es opcional y siempre puede escribir la dirección manualmente. YardSync guarda únicamente el texto de la dirección resultante, no las coordenadas precisas. Consulte la Sección 14.' : 'Location data. When you type an address into an intake form, the address text is sent to our mapping provider (Geoapify) to suggest matches. If you tap "Use my current location," your browser asks your permission and, if granted, your device\'s precise location is sent to Geoapify to convert it into an address. This feature is optional and you can always type the address manually. YardSync stores only the resulting address text, not the precise coordinates. See Section 14.'}</li>
            <li>{es ? 'Información de cuentas de miembros de equipo/cuadrilla ("trabajadores"), incluyendo el nombre, teléfono o correo con que se les invita, y qué trabajos se les asignan. Un trabajador ve únicamente el nombre del cliente, la dirección del servicio y la descripción del servicio de sus trabajos asignados; nunca ve datos de pago ni la lista completa de clientes' : 'Team member ("crew"/"worker") account information, including the name, phone, or email used to invite them, and which jobs they are assigned. A worker sees only the client name, service address, and service description for their assigned jobs; they never see payment data or the full client book'}</li>
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="list-disc pl-5 space-y-1">
            ...
            {/* PENDING LEGAL REVIEW (added 2026-07-05): Geoapify address lookup + crew/worker access purposes. */}
            <li>{es ? 'Para ofrecer autocompletado de direcciones y, si usted lo solicita, convertir la ubicación de su dispositivo en una dirección enviándola a nuestro proveedor de mapas (Geoapify), de modo que las direcciones de servicio se ingresen con exactitud' : 'To provide address autocomplete and, if you request it, to convert your device location into an address by sending it to our mapping provider (Geoapify), so service addresses are entered accurately'}</li>
            <li>{es ? 'Para permitir que un propietario invite a miembros de su equipo y les asigne trabajos, y para mostrar a cada trabajador únicamente los trabajos asignados que necesita para realizar su labor' : 'To let a business owner invite team members and assign them jobs, and to show each worker only the assigned jobs they need to do their work'}</li>
          </ul>
        </div>
        ...
        {/* INTERIM — MARKET-ALIGNED, PENDING LEGAL REVIEW (updated 2026-07-05): subprocessor flow-down sentence added — standard customer-facing "service providers are contractually limited to using data only to provide services to us" clause (cf. Housecall Pro Terms: service providers given only the personal info necessary to perform the service). NOT a full DPA / CCPA service-provider addendum — those remain lawyer-gated (risk register #15). */}
        <div>
          <h2 className="text-lg font-semibold text-gray-900 mb-2">{es ? '6. Compartir datos' : '6. Data Sharing'}</h2>
          <p>{es
            ? 'No vendemos su información personal. Los datos de suscripción móvil y los números de teléfono recopilados para comunicaciones por SMS nunca se compartirán con terceros ni afiliados con fines de marketing o promoción. Solo compartimos datos con proveedores de confianza necesarios para operar la plataforma (Firebase, Stripe, Twilio, Anthropic, Geoapify). Estos proveedores reciben únicamente la información personal necesaria para prestarnos su servicio, están obligados por contrato a usar esa información solo para proporcionarnos servicios a nosotros —no para sus propios fines— y a protegerla. No compartimos datos con terceros con fines de marketing. Los detalles de la tarjeta de su suscripción son almacenados por Stripe bajo los términos de Stripe; YardSync solo almacena un identificador de cliente y un token del método de pago, no el número de su tarjeta.'
            : 'We do not sell your personal information. Mobile opt-in data and phone numbers collected for SMS communications will never be shared with third parties or affiliates for marketing or promotional purposes. We share data only with trusted service providers necessary to operate the platform (Firebase, Stripe, Twilio, Anthropic, Geoapify). These service providers receive only the personal information necessary to perform their service for us, are contractually bound to use that information solely to provide services to us — not for their own purposes — and to protect it. We do not share data with third parties for marketing purposes. Card details for your subscription are stored by Stripe under Stripe’s terms; YardSync stores only a customer identifier and a payment-method token, not your card number.'}</p>
        ...
        {/* INTERIM — MARKET-ALIGNED, PENDING LEGAL REVIEW (updated 2026-07-05): Geoapify sub-processor + precise geolocation. Framed toward market norm for user-initiated address lookup: optional, opt-in, purpose-disclosed, manual entry always available, no coordinate retention. Note our stance is MORE protective than comparable field-service apps — Housecall Pro auto-collects mobile GPS with only a disable-after option; Jobber-linked policies collect location "with your consent." We treat precise device location as opt-in sensitive data (tap the button + grant the native browser prompt = clear affirmative act) consistent with TX TDPSA sensitive-data treatment, and reference the point-of-capture in-form notice. Counsel to confirm sufficiency of the affirmative-act consent + YardSync's facilitator-vs-controller role for coordinates it never stores. */}
        <div>
          <h2 className="text-lg font-semibold text-gray-900 mb-2">{es ? '14. Autocompletado de Direcciones y Ubicación (Geoapify)' : '14. Address Autocomplete and Location (Geoapify)'}</h2>
          <p>{es
            ? 'Para ayudar a ingresar direcciones de servicio con exactitud, YardSync usa a Geoapify, un proveedor externo de mapas y geocodificación. Cuando usted escribe una dirección en un formulario de captación de prospectos, el texto que escribe se envía a Geoapify para sugerir coincidencias. El formulario también ofrece un botón opcional de "Usar mi ubicación actual"; si usted lo toca, su navegador le pide permiso y, solo si lo concede, la ubicación precisa de su dispositivo (coordenadas) se envía a Geoapify para convertirla en una dirección legible (geocodificación inversa).'
            : 'To help enter service addresses accurately, YardSync uses Geoapify, a third-party mapping and geocoding provider. When you type an address into a prospect-intake form, the text you type is sent to Geoapify to suggest matches. The form also offers an optional "Use my current location" button; if you tap it, your browser asks for your permission and, only if you grant it, your device\'s precise location (coordinates) is sent to Geoapify to convert into a readable address (reverse geocoding).'}</p>
          <p className="mt-2">{es
            ? 'El uso de la ubicación es totalmente opcional y requiere su consentimiento afirmativo: nunca se captura la ubicación precisa de su dispositivo a menos que usted toque el botón y otorgue el permiso del navegador, y siempre puede escribir la dirección manualmente. En el punto en que toca "Usar mi ubicación actual", una nota breve en el formulario le indica que su ubicación se compartirá con nuestro proveedor de mapas, Geoapify. Tratamos la ubicación precisa del dispositivo como dato sensible que solo se recopila mediante este acto afirmativo de su parte. YardSync guarda únicamente el texto de la dirección resultante; no almacenamos las coordenadas precisas de su dispositivo. Geoapify actúa como nuestro proveedor de servicios (procesador) bajo contrato y ofrece un Acuerdo de Procesamiento de Datos; el texto de la dirección y las coordenadas que se comparten con Geoapify se procesan según la Política de Privacidad y los Términos de Geoapify, disponibles en '
            : 'Use of location is entirely optional and requires your affirmative opt-in: your device\'s precise location is never captured unless you tap the button and grant the browser permission, and you can always type the address manually. At the point where you tap "Use my current location," a brief in-form notice tells you that your location will be shared with our mapping provider, Geoapify. We treat precise device location as sensitive data collected only through this affirmative act by you. YardSync stores only the resulting address text; we do not store your device\'s precise coordinates. Geoapify acts as our service provider (processor) under contract and makes a Data Processing Agreement available; the address text and coordinates shared with Geoapify are processed under Geoapify\'s own Privacy Policy and Terms, available at '}<a href="https://www.geoapify.com/privacy-policy/" className="text-green-700 underline" target="_blank" rel="noopener noreferrer">geoapify.com/privacy-policy</a>.</p>
        </div>
