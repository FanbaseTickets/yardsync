...
> Keep it current. Last updated: 2026-06-20 (LIVE in production. Massive QA + Settings day — PRs #21–#28 merged: full QA hardening series (payments `on_behalf_of` fix, invoice auth/validation/double-send guard, strict NANP phone, trust-state webhook increment, recurring/double-booking guards, intake validation + contractor email/SMS lead notification, Accept-lead-requires-package, schedule start-date field, dashboard fix, honest SMS toasts, walk-in price polish); INFRA-1 permanent `dev` webhook alias; Settings tab refactor (Profile·Card·SMS·Billing + ?tab=) + Card/SMS/Billing polish; Profile overhaul (headshot upload, images-at-top, unified edit/lock + greying all tabs, logo/headshot casing fix); Firebase config trim (storageBucket stray-tab defense); free-estimate badge. PR #29 (landing "free digital business card" pitch + Terms/Privacy audit) merging at EOD. **Workflow changed: all work now on the permanent `dev` branch → PR `dev`→`main` → merge. Test Stripe webhook points at the stable `dev` Vercel alias (INFRA-1). Read the `session-pickup-latest` memory first each session.** Next: Tier 2 in docs/BACKLOG.md.).
>
